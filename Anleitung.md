...
Liegt ein Griff höher als der dritte Bund, ist der Sattel nicht mehr dargestellt und die **Bundlage** steht als kleine Zahl neben dem obersten Bund.

Die Griffe kommen aus drei Quellen, in dieser Reihenfolge: einer von Hand geprüften Tabelle mit den gebräuchlichen Akkorden, dann einer eingebauten **Griffdatenbank** mit 781 Akkorden in 3103 von Hand ausgesuchten Lagen ([chords-db](https://github.com/tombatossals/chords-db) von David Rubert, MIT-Lizenz), und erst für alles Übrige berechnet — meist, indem eine bekannte Form derselben Akkordart verschoben wird, so wie man es selbst tut. Jeder Griff aus der Datenbank durchläuft dabei dieselbe Prüfung wie ein berechneter: keine akkordfremden Töne, richtige Bassnote, höchstens vier Finger, höchstens drei Bünde Spreizung. Bleibt ein Akkord ganz ohne Bild, steht er trotzdem an seiner Stelle in der Leiste: mit **Namen** wie jeder andere, darüber ein leeres, blass gezeichnetes Griffbrett. Ein Klick darauf öffnet den Griffeditor, sodass der fehlende Griff gleich dort eingetragen werden kann. Zusätzlich ist der Akkord im Quelltext orange gewellt unterstrichen — fast immer steckt ein Schreibfehler dahinter, und das ist der Hinweis, wo nachzusehen ist.

**Andere Lagen.** Unter dem Raster steht eine Reihe mit allen bekannten Lagen desselben Akkords, nach Bundlage geordnet — bei `Ab9` etwa vier, vom Sattel bis in den zehnten Bund. Ein Klick lädt eine davon ins Raster; gespeichert ist sie damit noch nicht, du kannst sie also erst noch abwandeln. Erst „Speichern" schreibt sie als `{define}` ins Liedblatt und macht sie damit zum Griff dieses Liedes — auf allen Geräten.

Welche Lage automatisch angezeigt wird, entscheidet die Datenbank; das ist meist die tiefste. Ob dir eine höhere lieber ist, hängt vom Stück und vom Stil ab — in Jazz-Notationen etwa stehen oft vierstimmige Griffe in der Mitte des Halses. Das lässt sich nicht allgemein ausrechnen, deshalb entscheidest du es hier mit einem Klick, und es bleibt so.
...
- **Notizleiste** (Button oben links) — die Liste aller offenen Notizen, untereinander statt als Tableiste. Ein Klick öffnet sie als Overlay; über das Pin-Symbol wird daraus eine feste Seitenspalte, neben der das Notizfenster entsprechend schmaler wird.
  ...
  - Die Einstellung bleibt gespeichert und gilt für das Gerät, nicht für eine einzelne Notiz.
  - **Herkunftsvermerk** — wer mit dem Mauszeiger auf einem Eintrag stehenbleibt, sieht, wie die Notiz entstanden ist: „Aus der Datei ‚Protokoll.md' geöffnet", „Von https://… geladen", „Aus dem PDF ‚Arbeitsblatt.pdf' erstellt", „Aus der Vorlage ‚Meeting-Notizen' erstellt", „Über die Synchronisierung hereingekommen" oder „Hier neu angelegt" — darunter Datum und Uhrzeit. Notizen, die vor dieser Funktion angelegt wurden, haben keinen Vermerk und zeigen dann auch keinen an.
